...
1. 🔴 **Backfill S20 (and the S18/S19 PC-page gap).** PC pages are the worst of it: [[Binks Stonevein]], [[Amanita de'Champignon]], [[Blarg]] and [[Aeolus]] have nothing written up for **S18, S19 or S20**; [[Kit Aluri]] and [[Darby Stonefeather]] are missing S20. Every affected page now carries an inline `⚠️ Gap` marker naming the specific missing beats. Also outstanding: **S20 House Rules** and **S20 Open Threads**.
...
3. 🔵 **Website ARC registries are behind — not touched by the S21 pass** (out of scope for this run; they live in the public `rectrixcaedere` repo, not this vault). Per the Convo 2 instructions the backlog was already at `archive.html` current through S17 / `session.html` stopping at S15 — **S16–S21 now need wiring into both `ARC` arrays**, plus the `archive.html` header Sessions count and Deepest Reach.
