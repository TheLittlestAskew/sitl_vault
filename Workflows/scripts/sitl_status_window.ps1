# ════════════════════════════════════════════════════════════════════
#  SITL Pipeline — persistent status window
# --------------------------------------------------------------------
...
#  sitl_pipeline_watch.js) plus a tail of _pipeline\watcher.log, polled
...
#
#  Launched (hidden host) by the watcher, or by Show-SITL-Status.cmd.
# ════════════════════════════════════════════════════════════════════

# ── Single instance: a second launch just exits ──────────────────────
$createdNew = $false
$mutex = New-Object System.Threading.Mutex($true, 'SITL_STATUS_WINDOW_SINGLETON', [ref]$createdNew)
if (-not $createdNew) { return }

Add-Type -AssemblyName PresentationFramework, PresentationCore, WindowsBase

# ── Paths (derive the vault root from this script's location) ────────
$vaultRoot  = Split-Path (Split-Path $PSScriptRoot)        # ...\sitl_vault
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="SITL Pipeline" Height="540" Width="470"
  ...
  <DockPanel>
    <Border DockPanel.Dock="Top" Background="#252731" Padding="16,12">
      <StackPanel>
        <TextBlock x:Name="HeaderTitle" Foreground="#E8E6E0" FontSize="16" FontWeight="SemiBold" Text="SITL Pipeline"/>
...
function Refresh {
  ...
  $HeaderTitle.Text = if ($st.session) { "SITL Pipeline  " + [char]0x00B7 + "  S$($st.session)" } else { 'SITL Pipeline' }
  # Taskbar button text reflects state even when minimized
  $win.Title = 'SITL ' + [char]0x00B7 + ' ' + $sub
...
# Self-test hook: SITL_STATUS_SELFTEST=1 auto-closes after ~2.5s so the window
# can be exercised headlessly in CI/smoke runs. No effect in normal use.
if ($env:SITL_STATUS_SELFTEST -eq '1') {
